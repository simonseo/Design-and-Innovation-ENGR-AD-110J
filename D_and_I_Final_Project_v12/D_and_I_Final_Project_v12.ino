...
{ // Salinity Sensor 1
  ...
  // Happy
  else if (emotionIndex >= 0.45 && emotionIndex < 0.8) {

    if (millis() - start_time > 2000) {
      if (lastprinted != 3) {
        setColor(0, 100, 170); // Yellow
        Serial.println(3);
        lastprinted = 3;
        start_time = 0;
      }
  ...
  // in Love
  else if (emotionIndex >= 0.8 && emotionIndex < 1) {
    if (millis() - start_time > 2000) {
      if (lastprinted != 4) {
        setColor(0, 200, 0); // Pink
        Serial.println(4);
        lastprinted = 4;
        start_time = 0;
      }
